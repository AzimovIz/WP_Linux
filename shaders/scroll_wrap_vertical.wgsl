// Continuous vertical pixel scroll with seamless wraparound, looped
// within a chosen [u_start, u_start + u_length] slice of the image on the
// y axis rather than the full 0..1 canvas -- meant to be masked (Texture
...
// Sign of `u_speed` picks the direction: positive scrolls bottom-to-top,
// negative scrolls top-to-bottom. Pixels outside [u_start, u_start +
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let local = (in.uv.y - params.u_start) / params.u_length;
    let looped = fract(local + params.time * params.u_speed);
    let scroll_y = params.u_start + looped * params.u_length;
    return textureSample(input_texture, input_sampler, vec2<f32>(in.uv.x, scroll_y));
}
