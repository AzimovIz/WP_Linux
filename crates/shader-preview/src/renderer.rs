//! Offscreen render target + CPU readback on top of `player::SceneRenderer`.
//!
//! Deliberately a byte-for-byte copy of `render-server`'s own
//! `renderer.rs`, not a shared dependency -- `player` and `render-server`
//! are kept untouched on purpose (see `Ideas.md`'s discussion of this
//! tool), so this ~120-line offscreen-canvas helper is duplicated here
//! rather than factored out. If it ever needs to change, check whether
//! `render-server/src/renderer.rs` needs the same fix.

use player::wgpu;
pub use player::{LoadedLayer, SceneRenderer};

/// `player::OFFSCREEN_FORMAT` under a locally meaningful name.
...
/// size, once, so rendering many frames in a row isn't churning fresh GPU
